...
function Home() {
  ...
  const COLORS = ["#cc1b59", "#ebd002", "#48de1f", "#0b5fbf"];
  ...
  graphResultData.filter((element) => {
    if (
      element.price.$numberDecimal > 0 &&
      element.price.$numberDecimal <= 100
    ) {
      range1 = range1 + 1;
    } else if (
      element.price.$numberDecimal > 100 &&
      element.price.$numberDecimal <= 200
    ) {
      range2 = range2 + 1;
    } else if (
      element.price.$numberDecimal > 200 &&
      element.price.$numberDecimal <= 300
    ) {
      range3 = range3 + 1;
    } else if (
      element.price.$numberDecimal > 300 &&
      element.price.$numberDecimal <= 400
    ) {
      range4 = range4 + 1;
    } else if (
      element.price.$numberDecimal > 400 &&
      element.price.$numberDecimal <= 500
    ) {
      range5 = range5 + 1;
    } else if (
      element.price.$numberDecimal > 500 &&
      element.price.$numberDecimal <= 600
    ) {
      range6 = range6 + 1;
    } else if (
      element.price.$numberDecimal > 600 &&
      element.price.$numberDecimal <= 700
    ) {
      range7 = range7 + 1;
    } else if (
      element.price.$numberDecimal > 700 &&
      element.price.$numberDecimal <= 800
    ) {
      range8 = range8 + 1;
    } else if (
      element.price.$numberDecimal > 800 &&
      element.price.$numberDecimal <= 900
    ) {
      range9 = range9 + 1;
    } else if (element.price.$numberDecimal > 900) {
      range10 = range10 + 1;
    }
  });
    ...
    <div>
      ...
      <div className="body-container">
        <div className="table-structure">
          <h1>
            <b>Transaction table -</b> {getMonthName(selectedMonth)}
          </h1>
          {/* Pagination */}
          <nav>
        ...
        <div className="row">
          ...
          <div className="col-md-5 container-box check">
            <h1>
              <b>Bar Chart Stats -</b> {getMonthName(selectedMonth)}
            </h1>
            <ResponsiveContainer width="100%" height="85%">
              <BarChart
                data={barData}
                margin={{ top: 0, right: 0, left: 4, bottom: 50 }}
              >
